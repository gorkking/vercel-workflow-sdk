...
The generated workflow ID uses the form `workflow//dynamic/<id-or-hash>//<exportName>`. The `id` field is optional. Use it when you want a stable, human-readable workflow ID segment for queue names, run filtering, and observability. If you omit `id`, Workflow SDK derives one from the source and step references.
...
`dynamic.id` is optional. It does not affect the workflow logic. It only controls the stable ID segment in the generated workflow name. When omitted, Workflow SDK uses a hash of the source and step references.
