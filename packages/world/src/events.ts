 ...
 * that a noop's `createdAt` — the sealer's wall clock, which can postdate
 * every real event around it — never becomes a time the run observed.
 */
...
/**
 * Worlds may omit a payload field when an event is read with
 * `resolveData: 'none'`. Restore the field as `undefined` before validating a
