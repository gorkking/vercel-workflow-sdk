...
export function createQueue(config: Partial<Config>): LocalQueue {
  ...
  const createQueueHandler: Queue['createQueueHandler'] = (prefix, handler) =>
    createFetchQueueHandler(prefix, async (message, meta) => {
      const result = await handler(message, meta);
      // WORKFLOW_LOCAL_QUEUE_MAX_VISIBILITY caps redelivery delays (testing).
      if (result && result.timeoutSeconds > LOCAL_QUEUE_MAX_VISIBILITY) {
        return { timeoutSeconds: LOCAL_QUEUE_MAX_VISIBILITY };
      }
      return result;
    });
